      ...
      if (!state.candle.open) {
        ...
        if (state.ticker % 120 == 0) {
          ...
          state.candle = {
            ...
            time,
            close: action.payload,
            high: Math.max(state.candle.high, action.payload),
            low: Math.min(state.candle.low, action.payload),
          };
        }
      }
...
export const { resetChart, setChart, addTicker, resetTicker } =
  candleChart.actions;
